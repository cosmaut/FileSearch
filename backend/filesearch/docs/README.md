...
FileSearch 是一个高性能的网盘资源搜索 API 服务，支持 TG 搜索和自定义插件搜索。系统设计以性能和可扩展性为核心，支持并发搜索、结果智能排序和网盘类型分类。

> 当前推荐部署仓库：`https://github.com/cosmaut/FileSearch`
>
> 原始后端上游参考：`https://github.com/fish2018/pansou`
...
FileSearch 还提供了一个基于 [Model Context Protocol (MCP)](https://modelcontextprotocol.io) 的服务，可以将搜索功能集成到 Claude Desktop 等支持 MCP 的应用中。详情请参阅 [MCP 服务文档](docs/MCP-SERVICE.md)。
...
在 GitHub 上先从当前仓库开始部署：[cosmaut/FileSearch](https://github.com/cosmaut/FileSearch)
本项目，并点上 Star !!!
...
[当前仓库 Issues](https://github.com/cosmaut/FileSearch/issues)  
[插件与搜索源说明](https://github.com/cosmaut/FileSearch)
...
```bash
docker build -t filesearch:latest ../../
docker run -d --name filesearch -p 3200:3200 filesearch:latest
```

##### 使用Docker Compose（推荐）
```
# 下载配置文件
curl -o docker-compose.yml https://raw.githubusercontent.com/cosmaut/FileSearch/main/docker-compose.yml
...
```bash
docker build -t filesearch-backend:latest .
docker run -d --name filesearch-backend -p 8888:8888 filesearch-backend:latest
```
...
```bash
# 下载配置文件
curl -o docker-compose.yml https://raw.githubusercontent.com/cosmaut/FileSearch/main/backend/filesearch/docker-compose.yml
...
cd FileSearch/backend/filesearch
```
...
FileSearch 支持可选的安全认证功能，默认关闭。开启后，所有API接口（除登录接口外）都需要提供有效的JWT Token。详见[认证系统设计文档](docs/认证系统设计.md)。
...
```bash
# 启用认证并配置单个用户
docker run -d --name filesearch-backend -p 8888:8888 \
  -e AUTH_ENABLED=true \
  -e AUTH_USERS=admin:admin123 \
  -e AUTH_TOKEN_EXPIRY=24 \
  filesearch-backend:latest
...
```linux
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -extldflags '-static'" -o filesearch-backend .
```
...
command=/home/work/filesearch/filesearch-backend
directory=/home/work/filesearch
autostart=true
autorestart=true
startsecs=5
startretries=3
...
server {
    ...
    access_log /home/work/logs/filesearch.log;
...
[当前仓库地址](https://github.com/cosmaut/FileSearch)
