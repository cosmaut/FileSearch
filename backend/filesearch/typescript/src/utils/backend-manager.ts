...
/**
 * 后端服务管理器
 * 负责自动启动、停止和监控 FileSearch Go 后端服务
 */
export class BackendManager {
  ...
  private async detectDockerContainer(): Promise<boolean> {
    try {
      ...
      // 检查是否有运行中的后端容器
      const { stdout } = await execAsync('docker ps --format "{{.Names}}" --filter "name=filesearch"');
      const runningContainers = stdout.trim().split('\n').filter(name => name.includes('filesearch'));
  ...
  private async findGoExecutable(): Promise<string | null> {
    ...
    if (configProjectRoot) {
      possiblePaths.push(
        path.join(configProjectRoot, 'filesearch-backend.exe'),
        path.join(configProjectRoot, 'filesearch-backend'),
        path.join(configProjectRoot, 'filesearch.exe'),
        path.join(configProjectRoot, 'filesearch'),
      ...
      possiblePaths.push(
        // 当前工作目录
        path.join(process.cwd(), 'filesearch-backend.exe'),
        path.join(process.cwd(), 'filesearch-backend'),
        path.join(process.cwd(), 'filesearch.exe'),
        path.join(process.cwd(), 'filesearch'),
  ...
  async startBackend(): Promise<boolean> {
    ...
    const execPath = await this.findGoExecutable();
    if (!execPath) {
      console.error('未找到 FileSearch 后端可执行文件');
      console.error('如果您使用Docker部署，请在MCP配置中设置 DOCKER_MODE=true');
      console.error('如果您使用源码部署，请确保在项目根目录下存在以下文件之一：');
      console.error('  - filesearch-backend.exe / filesearch-backend');
      console.error('  - filesearch.exe / filesearch');
