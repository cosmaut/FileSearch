...
export const healthTool: Tool = {
  ...
  description: '检查FileSearch服务的健康状态，获取服务信息、可用插件和频道列表',
  inputSchema: {
    type: 'object',
    properties: {},
    required: []
...
function formatErrorResult(errorMessage: string, serverUrl: string): string {
  let output = `**FileSearch服务健康检查失败**\n\n`;
  
  output += `**服务地址**: ${serverUrl}\n`;
  output += `**错误信息**: ${errorMessage}\n\n`;
  
  output += `**可能的解决方案**:\n`;
  output += `   1. 检查FileSearch服务是否正在运行\n`;
  output += `   2. 确认服务地址配置是否正确\n`;
  output += `   3. 检查网络连接是否正常\n`;
  output += `   4. 查看服务日志获取更多信息\n\n`;
  
  output += `**配置说明**:\n`;
  output += `   可通过环境变量 FILESEARCH_SERVER_URL 配置服务地址\n`;
  output += `   兼容旧变量: PANSOU_SERVER_URL\n`;
