...
const AI_REQUIRE_CAPTCHA =
  (process.env.NEXT_PUBLIC_AI_SUGGEST_REQUIRE_CAPTCHA ?? "false").toLowerCase() === "true";
...
export async function POST(request: NextRequest) {
  ...
  if (AI_REQUIRE_CAPTCHA) {
    if (CAPTCHA_PROVIDER === "none") {
      return NextResponse.json(
        { message: "Captcha is required but provider is disabled" },
        { status: 400 },
      );
    }

    const captcha = await verifyCaptchaToken(request, captchaToken);
    if (!captcha.ok) {
      return NextResponse.json(
