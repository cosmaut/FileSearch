...
class FileSearchMCPServer {
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ListResourcesRequestSchema, async () => {
      return {
        resources: [
          {
            uri: 'filesearch://plugins',
            name: '可用插件列表',
            description: '获取当前可用的搜索插件列表',
            mimeType: 'application/json',
          },
          {
            uri: 'filesearch://channels',
            name: '可用频道列表',
            description: '获取当前可用的TG频道列表',
            mimeType: 'application/json',
          },
          {
            uri: 'filesearch://cloud-types',
            name: '支持的网盘类型',
    ...
    this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
      ...
      try {
        switch (uri) {
          case 'filesearch://plugins':
            return await this.getPluginsResource();

          case 'filesearch://channels':
            return await this.getChannelsResource();

          case 'filesearch://cloud-types':
  ...
  public async start(): Promise<void> {
    ...
    console.error('FileSearch MCP服务器已启动');
...
async function main(): Promise<void> {
  try {
    const server = new FileSearchMCPServer();
    await server.start();
  } catch (error) {
