...
FileSearch 是一个高性能的开源网盘资源搜索工具，当前仓库用于维护自己的部署版本。

> 当前仓库地址：`https://github.com/cosmaut/FileSearch`
>
> 原项目地址：`https://github.com/Maishan-Inc/Limitless-search`
>
> 当前版本：`0.1.0`
...
- 前端默认不再依赖 `web/filesearch_web/.env`
...
> 需要更多语言？请提交 [Issues](https://github.com/cosmaut/FileSearch/issues)
...
# SSH
git clone git@github.com:cosmaut/FileSearch.git

# GitHub CLI
gh repo clone cosmaut/FileSearch
```
...
```bash
cp web/filesearch_web/.env.example web/filesearch_web/.env.local
```
...
> **注意**：Docker 部署时无需创建 `web/filesearch_web/.env`。只有在本地开发前端时，才需要按需创建 `web/filesearch_web/.env.local`。
...
- [后端详细文档](backend/filesearch/docs/README.md)
- [插件开发指南](backend/filesearch/docs/插件开发指南.md)
- [系统设计文档](backend/filesearch/docs/系统开发设计文档.md)
