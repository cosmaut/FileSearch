...
FileSearch MCP 服务是一个基于 [Model Context Protocol (MCP)](https://modelcontextprotocol.io) 的工具服务，它将 FileSearch 网盘搜索 API 的功能封装为可在支持 MCP 的客户端（如 Cherry Studio）中直接调用的工具。

通过 FileSearch MCP 服务，可以直接在 Claude 等 AI 助手中搜索网盘资源，极大地提升了获取网盘资源的便捷性。
...
2. **检查服务健康状态 (`check_service_health`)**:
   - 检查所连接的 FileSearch 后端服务是否正常运行。
   - 获取后端服务的配置信息，如可用的 Telegram 频道列表和插件列表。

3. **启动后端服务 (`start_backend`)**:
   - 自动启动本地的 FileSearch Go 后端服务（如果尚未运行）。
   - 等待服务完全启动并可用后才开始处理其他请求。
   - 支持参数：`force_restart`（可选，布尔值，是否强制重启后端服务，默认为false）。

4. **获取静态资源信息 (`filesearch://` URI scheme)**:
   - 提供可用插件列表、可用频道列表和支持的网盘类型列表等静态信息资源。
   - 支持资源URI：`filesearch://plugins`（插件列表）、`filesearch://channels`（频道列表）、`filesearch://cloud-types`（网盘类型列表）。

### 架构与部署方式

FileSearch MCP 服务设计为与 FileSearch Go 后端服务分离，通过 HTTP API 进行通信。支持以下部署方式：

- **Node.js 部署 (TypeScript)**: MCP 服务基于 TypeScript 开发，编译后通过 `node` 命令运行编译后的 JavaScript 文件。它会自动连接到指定的 FileSearch 后端服务。
- **Docker 部署**: 使用 Docker 容器运行 FileSearch 后端服务，MCP 服务通过 HTTP API 连接到容器化的后端。
...
FileSearch 后端服务通常运行在 `http://localhost:8888` (默认地址)。支持以下两种后端部署方式：
...
- 克隆或确保已有 FileSearch Go 项目源码。
- 在项目根目录下，打开终端并执行以下命令进行构建：

```bash
# Windows (PowerShell/CMD)
go build -o filesearch-backend.exe .
```
...
在 FileSearch 项目根目录下，使用 Docker Compose 启动后端服务：
...
- **使用 `node` 直接运行** (手动启动):
  在 FileSearch 项目根目录下（包含 `typescript` 文件夹），运行：
...
服务启动后，将默认尝试连接到 `http://localhost:8888` 的 FileSearch 后端服务。

如果想要后端服务运行在不同的地址或端口上，需要通过环境变量指定：

```bash
# Windows (CMD)
set FILESEARCH_SERVER_URL=http://your-backend-address:port
node .\typescript\dist\index.js

# Windows (PowerShell)
$env:FILESEARCH_SERVER_URL='http://your-backend-address:port'
node .\typescript\dist\index.js
```
...
要在 Cherry Studio 中使用 FileSearch MCP 服务，需要将其添加到 Cherry Studio MCP 的配置文件中。
...
```json
{
  "mcpServers": {
    "filesearch": {
      ...
      "env": {
        "FILESEARCH_SERVER_URL": "http://localhost:8888",
        "REQUEST_TIMEOUT": "30",
        "MAX_RESULTS": "50",
...
配置完成后，在对话界面启用 FileSearch MCP 服务，即可开始尝试搜索。
...
1. **Docker 容器检测**：检查是否有运行中的 Docker 容器（名称包含 "filesearch"）
2. **源码部署检测**：检查是否存在 Go 可执行文件（filesearch-backend.exe/main.exe）
...
| 环境变量               | 描述                                                       | 默认值                    |
| :--------------------- | :--------------------------------------------------------- | :------------------------ |
| `FILESEARCH_SERVER_URL` | FileSearch 后端服务的 URL 地址。                               | `http://localhost:8888`   |
...
| `PROJECT_ROOT_PATH`    | FileSearch 后端可执行文件所在的目录路径（用于自动启动）。      | 无                        |
